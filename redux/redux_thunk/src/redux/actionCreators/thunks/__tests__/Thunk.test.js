...
import { addPlayer, removePlayer, setPlayers, updatePlayer } from '../../playersActions';
import { clearSelectedPlayer, setSelectedPlayer } from '../../selectedPlayerActions';
import { setStatus } from '../../statusActions';
import { postPlayer } from '../AddPlayer';
import { getSelectedPlayer } from '../ListPlayer.js';
import { getPlayers } from '../ListPlayers.js';
import { deleteSelectedPlayer, updateSelectedPlayer } from '../SelectedPlayer.js';
...
describe('Testing thunk action creators', () => {
  describe('postPlayer:', () => {
    ...
    it('should dispatch setStatus(REQ_STATUS.loading), setStatus(REQ_STATUS.error) action on unsuccessful requests', async () => {
      ...
      const expectedActions = [setStatus(REQ_STATUS.loading), setStatus(REQ_STATUS.error)];
      await store.dispatch(postPlayer(expectedBody));
      const actualActions = store.getActions();
      expect(actualActions).toEqual(expectedActions);
    });
  ...
  describe('updateSelectedPlayer:', () => {
    ...
    it('should dispatch setStatus(REQ_STATUS.loading), setStatus(REQ_STATUS.error) action on unsuccessful requests', async () => {
      ...
      const expectedActions = [setStatus(REQ_STATUS.loading), setStatus(REQ_STATUS.error)];
      await storeWithPlayer.dispatch(updateSelectedPlayer(expectedBody.isActive));
      const actualActions = storeWithPlayer.getActions();
      expect(actualActions).toEqual(expectedActions);
    });
  });
