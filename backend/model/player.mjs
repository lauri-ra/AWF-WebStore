...
const validatePlayer = (data, mode = 'update') => {
  ...
  const result = validator.validate({ ...data }, { convert: true, abortEarly: false });
...
const getPlayerIndex = id => {
  ...
  if (!('players' in db.data)) throw new PlayerNotFoundError('Player does not exist!');
...
export const createPlayer = async data => {
  ...
  if (!('metaData' in db.data)) db.data.metaData = { players: { nextPlayerId: 1 } };
  if (!('players' in db.data.metaData)) db.data.metaData.players = { nextPlayerId: 1 };
