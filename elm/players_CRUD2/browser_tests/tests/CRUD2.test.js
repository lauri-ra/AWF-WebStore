describe('Players CRUD2', function () {
  ...
  const setupServer = () => {
    ...
    server.respondWith('GET', /\/api\/players\/(\d+)$/, (xhr, id) => {
      const player = playersData.find(player => player.id === Number.parseInt(id));
      ...
      xhr.respond(200, { 'Content-Type': 'application/json' }, JSON.stringify(player));
    });
    ...
    server.respondWith('GET', /\/api\/players\/$/, xhr => {
      xhr.respond(200, { 'Content-Type': 'application/json' }, JSON.stringify(playersData));
    });
  ...
  it('should fetch player data from the backend with a GET request', async function () {
    ...
    chai.expect(server.requests.length).to.equal(1, 'Did not receive a request to the backend API');
    chai.expect(server.requests[0].url).to.match(/\/api\/players\/$/, 'Incorrect API address');
  ...
  it('should populate #players-list with fetched players', async function () {
    ...
    listedPlayers.forEach((playerItem, i) => {
      ...
      const statusInput = playerItem.querySelector('.player-status');
  ...
  it('should send new player data to the backend in a POST request', async function () {
    ...
    chai.expect(server.requests.length).to.equal(2, 'Did not receive a request to the backend API');
    chai.expect(server.requests[1].url).to.match(/\/api\/players\/$/, 'Incorrect API address');
  ...
  it('should add new player to the end of #players-list with status "not active"', async function () {
    ...
    chai
      ...
      .to.be.equal('player-4', 'Id attribute of the list item should be of the form "player-<ID>"');

    const nameDiv = lastPlayer.querySelector('.player-name');
    const statusInput = lastPlayer.querySelector('.player-status');

    chai.expect(nameDiv).to.not.be.null;
    chai.expect(statusInput).to.not.be.null;

    chai.expect(nameDiv.textContent.trim()).to.equal('Player Four', 'Player name is incorrect.');
  ...
  it('should send player status change to the backend in a PUT request', async function () {
    ...
    // Select Player Two
    const statusInput = listedPlayers[1].querySelector('.player-status');
    ...
    chai.expect(server.requests[1].url).to.match(/\/api\/players\/\d+$/, 'Incorrect API address');

    chai.expect(server.requests[1].method).to.equal('PUT', 'Player editing should use PUT request');
  ...
  it('should delete player from #players-list when "Delete" button is clicked', async function () {
    ...
    const deleteButton = document.querySelector('#players-list li#player-2 .btn-delete');
    deleteButton.click();
    await delay(100);
    ...
    listedPlayers.forEach(playerItem => {
      ...
      chai
        .expect(nameDiv.textContent.trim())
        .to.not.equal(playersData[1].name, 'Wrong player was deleted from the list');
    });
  });
});
